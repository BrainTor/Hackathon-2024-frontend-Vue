    ...
    <div class="containerInside shadowContainer">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;">
        ...
        <div class="inputRow">
          ...
          <input v-model="loginInput" class="insideInput">
        </div>
        <p class="insideText">Введите Пароль</p>
        <div class="inputRow">
          <fa class="input-icon" icon="lock" />
          <input v-model="passwordInput" type="password" class="insideInput">
        </div>
      ...
      <div style="display:flex;flex-direction:column; align-items:center;">
        <button id="submit" @click="onSubmitClicked">Войти</button>
        <div style="display:flex;flex-direction:row; justify-content:space-between;">
          <p class="insideText" style="font-size:1rem; margin-bottom:1.5rem">Нет аккаунта?</p>

          <router-link to="/register" class="insideText"
            style="font-family: 'Noto Sans', sans-serif;font-size:1rem; margin-bottom:1.5rem;color:var(--primary_accent);text-decoration: underline; margin-left:7px;cursor:pointer;"><a>
              Зарегистрироваться</a></router-link>
...
#submit {
  ...
  color: var(--backgroundTextButtonSuc);
  height: 45px;
  font-size: 1.21rem;
  margin-top: 1rem;
  border: 0px;
...
export default defineComponent({
  ...
  methods: {
    async onSubmitClicked() {
      const credential = this.loginInput;
