<template>
    <div class = "mainContainer animBlink">
        <div class= "codeContainer shadowContainer">
            ...
            <p style = "margin-left:20px">Код подтверждения отправлен на указанную почту </p>

            <div class = "inputsRow">
                <input  v-for="(input, index) in inputs" :key="index" type="text" ref="inputs" v-model="input.value" @paste="handlePaste($event)" @input="handleInput(index, $event)" maxlength="1" class = "inputCode">  
            </div>
            <p v-if="!this.timerEnd" :class = "{startAnim:!timerEnd}" style = "margin-left:20px; cursor:not-allowed">Не пришел код? Отправить повторно,через {{ formattedTime }}</p>
...
export default defineComponent({
    ...
    computed:{
    formattedTime(){
      return `${this.minutes.toString().padStart(2, '0')}:${this.secundes.toString().padStart(2, '0')}`;
    }
  },
    data(){
        return {
